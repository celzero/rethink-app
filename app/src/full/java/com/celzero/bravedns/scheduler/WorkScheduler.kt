...
class WorkScheduler(val context: Context) {
    ...
    fun scheduleAppExitInfoCollectionJob() {
        if (isWorkScheduled(context, APP_EXIT_INFO_JOB_TAG) || isWorkRunning(
                context,
        ...
        val appExitInfoCollector = PeriodicWorkRequest.Builder(
    ...
    fun scheduleDatabaseRefreshJob() {
        ...
        val refreshAppsJob = PeriodicWorkRequest.Builder(
    ...
    fun scheduleOneTimeWorkForAppExitInfo() {
        ...
        val appExitInfoCollector =
            OneTimeWorkRequestBuilder<AppExitInfoCollector>().setBackoffCriteria(
                BackoffPolicy.LINEAR, OneTimeWorkRequest.MIN_BACKOFF_MILLIS,
                TimeUnit.MILLISECONDS
            ).addTag(APP_EXIT_INFO_ONE_TIME_JOB_TAG).build()
    ...
    fun scheduleBlocklistUpdateCheckJob() {
        if (isWorkScheduled(context, BLOCKLIST_UPDATE_CHECK_JOB_TAG) || isWorkRunning(
                context,
        ...
        val blocklistUpdateCheck = PeriodicWorkRequest.Builder(
