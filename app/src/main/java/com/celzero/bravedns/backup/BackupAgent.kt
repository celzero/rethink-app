...
class BackupAgent(val context: Context, workerParams: WorkerParameters) :
    ...
    private var filesToZip: MutableList<File> = ArrayList()
    ...
    private fun startBackupProcess(backupFileUri: Uri): Boolean {
        ...
        try {
            ...
            Log.e(
                LOG_TAG_BACKUP_RESTORE, "exception during backup process, reason? ${e.message}",
                e
            )
            return false
        } finally {
    ...
    private fun saveDatabasesToFile(path: String): Boolean {
        ...
        for (f in files) {
            val databaseFile = backUpDatabaseFile(
                f.absolutePath,
                constructDbFileName(path, f.name)
            ) ?: return false
            if (DEBUG) Log.d(
                LOG_TAG_BACKUP_RESTORE,
