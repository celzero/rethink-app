...
interface CountryConfigDAO {
    ...
    @Query("UPDATE CountryConfig SET enabled = :value, lastModified = :timestamp WHERE cc = :cc")
    ...
    @Query("SELECT COUNT(*) FROM CountryConfig WHERE enabled = 1")
    suspend fun getEnabledCount(): Int

    @Query("SELECT COUNT(*) FROM CountryConfig WHERE catchAll = 1 AND enabled = 1")
    suspend fun getCatchAllCount(): Int

    @Query("SELECT COUNT(*) FROM CountryConfig WHERE lockdown = 1 AND enabled = 1")
    ...
    @Query("SELECT EXISTS(SELECT 1 FROM CountryConfig WHERE cc = :cc AND enabled = 1)")
