...
private const val SORT_ORDER =
    " order by case when lower(:sort) = 'name' then lower(appName) end asc," +
            " case when lower(:sort) = 'package' then lower(packageName) end asc," +
            " case when lower(:sort) = 'uid' then uid end asc," +
            " lower(appName) asc"
    }
