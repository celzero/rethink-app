...
interface AppInfoDAO {
    ...
    @Query(
        "select * from AppInfo where isSystemApp = 1 and appName like :name order by isInternetAllowed,lower(appName)")
    fun getSystemAppInfoLiveData(name: String): DataSource.Factory<Int, AppInfo>

    @Query(
        "select * from AppInfo where isSystemApp = 1 and appName like :name and appCategory in (:filter) order by isInternetAllowed,lower(appName)")
    fun getSystemAppInfoLiveData(name: String,
                                 filter: Set<String>): DataSource.Factory<Int, AppInfo>

    @Query(
        "select * from AppInfo where isSystemApp = 0 and appName like :name order by isInternetAllowed,lower(appName)")
    fun getInstalledAppInfoLiveData(name: String): DataSource.Factory<Int, AppInfo>

    @Query(
        "select * from AppInfo where isSystemApp = 0 and appName like :name and appCategory in (:filter) order by isInternetAllowed,lower(appName)")
    fun getInstalledAppInfoLiveData(name: String,
                                    filter: Set<String>): DataSource.Factory<Int, AppInfo>

    @Query(
        "select * from AppInfo where appName like :name order by isInternetAllowed,lower(appName)")
    fun getAppInfosLiveData(name: String): DataSource.Factory<Int, AppInfo>

    @Query(
        "select * from AppInfo where appName like :name and appCategory in (:filter) order by isInternetAllowed,lower(appName)")
