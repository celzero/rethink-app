...
class PersistentState(context: Context) : SimpleKrate(context), KoinComponent {
    ...
    var udpBlockedSettings by booleanPref("block_udp_traffic_other_than_dns").withDefault<Boolean>(false)
    ...
    var blockUnknownConnections by booleanPref("block_unknown_connections").withDefault<Boolean>(false)
    ...
    var remoteBlocklistTimestamp by longPref("remote_block_list_downloaded_time").withDefault<Long>(INIT_TIME_MS)
    ...
    var allowBypass by booleanPref("allow_bypass").withDefault<Boolean>(!Utilities.isFdroidFlavour())
    ...
    var proxyType by stringPref("proxy_proxytype").withDefault<String>(AppConfig.ProxyType.NONE.name)

    // user selected proxy provider, as of now two providers (custom, orbot)
    var proxyProvider by stringPref("proxy_proxyprovider").withDefault<String>(AppConfig.ProxyProvider.NONE.name)
    ...
    var fetchFavIcon by booleanPref("fav_icon_enabled").withDefault<Boolean>(!Utilities.isFdroidFlavour())
    ...
    var useCustomDownloadManager by booleanPref("use_custom_download_managet").withDefault<Boolean>(true)

    // custom download manager's last generated id
    var customDownloaderLastGeneratedId by longPref("custom_downloader_last_generated_id").withDefault<Long>(0)

    var isLocalBlocklistUpdateAvailable by booleanPref("local_blocklist_update_check").withDefault<Boolean>(false)

    var isRemoteBlocklistUpdateAvailable by booleanPref("remote_blocklist_update_check").withDefault<Boolean>(false)

    // auto-check for blocklist update periodically (once in a day)
    var periodicallyCheckBlocklistUpdate by booleanPref("check_blocklist_update").withDefault<Boolean>(false)

    // user-preferred Internet Protocol type, default IPv4
    var internetProtocolType by intPref(INTERNET_PROTOCOL).withDefault<Int>(InternetProtocol.IPv46.id)
    ...
    var blockMeteredConnections by booleanPref("block_metered_connections").withDefault<Boolean>(false)
