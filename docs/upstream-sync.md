...
4. If it does NOT, file an issue in the fork to track the structural
   ...
   * shadow the affected file in `app/src/tv/java/...` until upstream
     stabilises, or
   * open an upstream issue / PR (see `for-upstream` branch — Phase
     9 in `plan.md`).
...
2. **Contribute-back**: the `for-upstream` branch (Phase 9 in
   `plan.md`) is generated by replaying our `app/src/tv/` adds on top
   of a fresh `upstream/main`. That replay only works if our diff
   against upstream stays minimal and additive.
