...
RPN is a multi-party relay, with connections existing over *serverless* proxy (hosted on Cloudflare Workers) hopping over Windscribe. Users are able to self-host *serverless* proxy or use the ones run by us. RPN starts at $1.75/month for unlimited bandwidth and supports activating 5 locations (out of 80+) in parallel, per-device.
